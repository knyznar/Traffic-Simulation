...
class Road {
    ...
    public static int[] nodes = {10,40,80};     //skrzyżowania
    public static int[] lights_off = new int[2];
    public static int[] lights_on = new int[1];
    ...
    private void move_single_car(Car car, ArrayList<Car> lane, int nrOfLine) {
        ...
        if (nrOfLine < 4) {
            ...
            if (red_light == 1 && position != lights_on[0] && car.getV_current() == 0 && position + 1 < lane.size())    //stoi na światłach
